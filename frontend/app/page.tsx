...
export default function Home() {
  ...
  async function upload(e: ChangeEvent<HTMLInputElement>) {
    ...
    try { const data = await request<UploadResponse>("/upload", { method: "POST", body: form }); setStudents(data.students); setReport(data.report); setPage(1); }
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-100"><div className="mx-auto max-w-7xl px-5 py-10 sm:px-8">
    ...
    {!report ? <section className="grid min-h-80 place-items-center rounded-2xl border border-dashed border-slate-700 bg-slate-900/50 p-8 text-center"><div><p className="text-lg font-medium">Upload a student CSV to begin</p><p className="mt-2 text-sm text-slate-400">Required: Name, Gender, Grade, Math, Science, English. Total is checked and recalculated.</p></div></section> : <>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900"><div className="flex items-center justify-between border-b border-slate-800 p-5"><div><h2 className="font-semibold">Cleaned roster</h2><p className="mt-1 text-sm text-slate-400">Toggle status to immediately include or exclude a student.</p></div><span className="text-xs text-slate-500">{students.length} records</span></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-950/50 text-xs uppercase tracking-wider text-slate-500"><tr>{["Student", "Gender", "Grade", "Math", "Science", "English", "Total", "Status"].map((h) => <th className="px-5 py-3 font-medium" key={h}>{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-800">{visibleStudents.map((s) => <tr key={s.id} className={s.status === "Debarred" ? "bg-rose-500/[.03] text-slate-500" : "hover:bg-slate-800/45"}><td className="px-5 py-3 font-medium text-slate-100">{s.name}</td><td className="px-5 py-3">{s.gender}</td><td className="px-5 py-3">{s.grade}</td><td className="px-5 py-3">{s.Math}</td><td className="px-5 py-3">{s.Science}</td><td className="px-5 py-3">{s.English}</td><td className="px-5 py-3 font-semibold text-cyan-300">{s.total}</td><td className="px-5 py-3"><button onClick={() => toggle(s)} className={`rounded-full px-3 py-1 text-xs font-bold ${s.status === "Active" ? "bg-emerald-400/15 text-emerald-300" : "bg-rose-400/15 text-rose-300"}`}>{s.status}</button></td></tr>)}</tbody></table></div><div className="flex flex-col gap-3 border-t border-slate-800 p-4 text-sm sm:flex-row sm:items-center sm:justify-between"><p className="text-slate-400">Showing {students.length ? pageStart + 1 : 0}–{Math.min(pageStart + pageSize, students.length)} of {students.length}</p><div className="flex items-center gap-3"><label className="text-slate-400">Rows <select aria-label="Rows per page" className="ml-1 rounded border border-slate-700 bg-slate-950 px-2 py-1 text-slate-200" value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }}><option value={25}>25</option><option value={50}>50</option><option value={100}>100</option></select></label><button aria-label="Previous page" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="rounded border border-slate-700 px-3 py-1 disabled:cursor-not-allowed disabled:opacity-40">Previous</button><span className="text-slate-300">Page {currentPage} of {pageCount}</span><button aria-label="Next page" onClick={() => setPage((p) => Math.min(pageCount, p + 1))} disabled={currentPage === pageCount} className="rounded border border-slate-700 px-3 py-1 disabled:cursor-not-allowed disabled:opacity-40">Next</button></div></div></section>
